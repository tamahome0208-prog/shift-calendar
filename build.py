...
:root {
  ...
  --shadow-xs: 0 1px 2px rgba(63, 123, 20, 0.05);
  --shadow-sm: 0 2px 6px rgba(63, 123, 20, 0.08);
  --shadow-md: 0 8px 24px rgba(63, 123, 20, 0.12);
  --shadow-lg: 0 20px 60px rgba(63, 123, 20, 0.22);
  --shadow-pink: 0 6px 20px rgba(255, 122, 165, 0.35);
  --shadow-leaf: 0 6px 20px rgba(91, 165, 40, 0.35);
...
body {
  ...
  background-image:
    radial-gradient(circle at 8% 14%, rgba(124, 194, 67, 0.06) 0%, transparent 30%),
    radial-gradient(circle at 92% 88%, rgba(255, 183, 211, 0.06) 0%, transparent 30%);
}
...
.mascot.hop { animation: mascotHop 0.6s var(--ease-spring); }
@keyframes mascotHop {
...
.month-info { display: flex; flex-direction: column; line-height: 1.05; }
.year-label {
...
.today-btn:active { transform: scale(0.92); }
...
.cell {
  ...
  overflow: hidden;
  transition: transform 0.18s var(--ease-spring), box-shadow 0.2s var(--ease-out), border-color 0.2s var(--ease-out);
  position: relative;
  opacity: 0;
...
.cell:active { transform: scale(0.96); box-shadow: var(--shadow-md); }
.cell.other-month {
  background: var(--surface-soft);
  border-color: transparent;
}
.cell.other-month .date-num { color: var(--muted); opacity: 0.5; }
.cell.today {
  border-color: var(--leaf);
  border-width: 2px;
  box-shadow: 0 0 0 4px rgba(124, 194, 67, 0.18), var(--shadow-sm);
}
.date-num {
  font-family: var(--font-en);
  font-size: 15px;
...
.cell.today .date-num {
  ...
  padding: 0;
  font-weight: 900;
  box-shadow: 0 2px 6px rgba(91, 165, 40, 0.45);
}
...
.event.kouki.t-outing { background: linear-gradient(135deg, var(--kouki-outing) 0%, var(--kouki) 110%); color: #0B3B5C; }
.event.kouki.t-outing .event-name { opacity: 0.75; color: #0B3B5C; }
...
.hero-card {
  ...
  background:
    radial-gradient(ellipse at top right, rgba(255, 183, 211, 0.18) 0%, transparent 55%),
    linear-gradient(135deg, #FFF8FB 0%, #FBFFF6 55%, #F0F8E3 100%);
  border: 1.5px solid var(--mint-soft);
  border-radius: 22px;
  box-shadow: var(--shadow-sm);
  position: relative;
  overflow: hidden;
  animation: heroIn 0.5s var(--ease-out);
}
@keyframes heroIn {
...
.hero-status.hot {
  background: linear-gradient(135deg, var(--cheek-deep) 0%, var(--yui) 100%);
...
.hero-stat:active { transform: scale(0.94); box-shadow: var(--shadow-sm); }
.hero-stat-label {
  font-size: 10px;
  color: var(--muted);
  font-weight: 800;
...
.hero-stat-value .unit { font-size: 11px; font-weight: 700; margin-left: 1px; }
.hero-stat-sub {
  font-size: 10.5px;
  color: var(--muted);
  font-weight: 700;
...
.hero-stat.date-stat { border-color: var(--lily-pink); }
.hero-stat.salary-stat .hero-stat-value { color: var(--leaf-dark); }
.hero-stat.salary-stat { border-color: var(--mint-soft); }
.hero-stat.count-stat .hero-stat-value { color: var(--yui); }
.hero-stat.count-stat { border-color: var(--yui-soft); }
...
@keyframes todayGlow {
  0%, 100% { box-shadow: 0 4px 12px rgba(255, 122, 165, 0.25); }
  50% { box-shadow: 0 6px 18px rgba(255, 122, 165, 0.45); }
}
  ...
  <div class="top-row">
    <div class="title-block">
      ...
      <div class="month-info">
        ...
        <div class="month-label"><span class="num" id="month-num"></span>月</div>
      </div>
    </div>
...
function renderHero() {
  ...
  } else if (nextDate && nextDate.days > 0 && nextDate.days <= 7) {
    statusMsg = `あと ${nextDate.days} 日で二人のおやすみ`;
  } else if (nextSalary && nextSalary.days > 0 && nextSalary.days <= 5) {
