...
const Storage = {
  ...
  async saveShifts(ym, events) {
    ...
    if (!firestoreDb || !navigator.onLine) {
      let local;
      try {
        local = JSON.parse(localStorage.getItem('sc_local_shifts') || '{}');
      } catch(e) {
        local = {};
      }
      local[ym] = payload;
  ...
  async initFirebase(cfg) {
    ...
    // v11.0 signals/pulse subscribe
    if (unsubscribePulse) unsubscribePulse();
    let __pulseInitialized = false;
    unsubscribePulse = onSnapshot(this._doc(firestoreDb, 'signals', 'pulse'), snap => {
      const data = snap.data();
      if (!data) return;
      const ts = data.ts?.toMillis ? data.ts.toMillis() : 0;
      // 初回発火はページロード時の既存 doc なので、baseline としてts記録のみ、通知は発火させない
      if (!__pulseInitialized) {
        __pulseInitialized = true;
        __lastPulseTs = ts;
        return;
      }
      const me = identityStore.get();
...
function renderHero() {
  ...
  for (let dd = 1; dd <= dim; dd++) {
    ...
    if (isBothOff(MERGED[k] || [])) monthCount++;
  }

  // 今日のステータスメッセージ
  const todayBuiltin = MERGED[todayKey] || [];
...
async function init() {
  ...
  if (reportDl) reportDl.onclick = () => {
    ...
    downloadBlob(__reportBlob, `${y}-${m}-report.png`);
  };
...
const ShiftPDFParser = {
  ...
  ROW_Y_TOLERANCE: 3,         // 同一行と判定する Y座標の許容 px
  MAX_COLUMN_X_DISTANCE: 30,  // 列 X座標から離れすぎのセル除外閾値 px
  ...
  _detectYearMonth(items) {
    // タイトルは先頭にあるため、先頭30itemsに限定してfooter等の誤検出を避ける
    const scoped = items.slice(0, 30);
    const joined = scoped.map(it => it.str).join(' ');
...
function mergeEventsWithShifts(builtinEvents, shifts) {
  const SHIFT_PERSONS = new Set(['こうき', 'ゆい']);
  const merged = { ...builtinEvents };
  for (const [ym, doc] of Object.entries(shifts)) {
    if (!doc || !doc.events) continue;
    // その月の既存キーから「こうき/ゆい」だけ除去(給料日など他 person は温存)
    Object.keys(merged).forEach(k => {
      if (!k.startsWith(ym + '-')) return;
      const kept = (merged[k] || []).filter(e => !SHIFT_PERSONS.has(e.person));
      merged[k] = kept.length ? kept : undefined;
      if (merged[k] === undefined) delete merged[k];
    });
    // Firestore の shift events を追加
...
const Report = {
  ...
  async drawCanvas(data, canvas) {
    ...
    // Header
    ctx.fillStyle = kouki;
    ctx.font = '900 68px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText(`${data.seasonEmoji} ${data.year}年 ${data.month + 1}月`, 60, 140);
    ctx.font = '700 40px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
    ctx.fillStyle = muted;
    ...
    cards.forEach((c, i) => {
      ...
      if (ctx.roundRect) {
        ...
        // roundRect fallback: 手動 path で角丸描画
        ctx.beginPath();
        ctx.moveTo(x + r, cardY);
        ctx.lineTo(x + cardW - r, cardY);
        ctx.quadraticCurveTo(x + cardW, cardY, x + cardW, cardY + r);
        ctx.lineTo(x + cardW, cardY + 300 - r);
        ctx.quadraticCurveTo(x + cardW, cardY + 300, x + cardW - r, cardY + 300);
        ctx.lineTo(x + r, cardY + 300);
        ctx.quadraticCurveTo(x, cardY + 300, x, cardY + 300 - r);
      ...
      ctx.fillStyle = 'white';
      ctx.font = '700 28px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(c.label, x + cardW / 2, cardY + 60);
      ctx.font = '900 120px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
      ctx.fillText(String(c.value), x + cardW / 2, cardY + 200);
      ctx.font = '700 32px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
    ...
    let y = 720;
    ctx.fillStyle = text;
    ctx.font = '900 44px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('ハイライト', 60, y);
    y += 60;
    ctx.font = '700 36px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
...
async function openReportSheet(year, month) {
  ...
  try {
    ...
    const previewUrl = URL.createObjectURL(__reportBlob);
    img.src = previewUrl;
...
function spawnOverlapCelebration(cellKey) {
  if (cellKey && /^\d{4}-\d{2}-\d{2}$/.test(cellKey)) {
    try {
...
EXTRA_EVENTS_BY_DATE = {
    ...
    "2026-08-01": [{"person": "ゆい", "summary": "希望休"}],
    "2026-08-02": [{"person": "ゆい", "summary": "希望休"}],
    "2026-08-03": [{"person": "こうき", "summary": "休"}],
    "2026-08-04": [{"person": "こうき", "summary": "会議"}],
    "2026-08-05": [{"person": "ゆい", "summary": "MTG"}],
    "2026-08-06": [{"person": "こうき", "summary": "会議"}],
    "2026-08-07": [{"person": "こうき", "summary": "休"}, {"person": "ゆい", "summary": "休"}],
    "2026-08-08": [{"person": "こうき", "summary": "出店"}, {"person": "ゆい", "summary": "希望休"}],
    "2026-08-09": [{"person": "こうき", "summary": "会議"}, {"person": "ゆい", "summary": "出店"}],
    "2026-08-10": [{"person": "こうき", "summary": "出店"}, {"person": "ゆい", "summary": "会議"}],
    "2026-08-11": [{"person": "こうき", "summary": "出店"}],
    "2026-08-12": [{"person": "こうき", "summary": "休"}, {"person": "ゆい", "summary": "休"}],
    "2026-08-13": [{"person": "こうき", "summary": "希望休"}, {"person": "ゆい", "summary": "希望休"}],
    "2026-08-14": [{"person": "こうき", "summary": "休"}],
    "2026-08-16": [{"person": "ゆい", "summary": "出店"}],
    "2026-08-17": [{"person": "こうき", "summary": "休"}, {"person": "ゆい", "summary": "希望休"}],
    "2026-08-18": [{"person": "こうき", "summary": "会議"}],
    "2026-08-19": [{"person": "こうき", "summary": "休"}, {"person": "ゆい", "summary": "休"}],
    "2026-08-20": [{"person": "こうき", "summary": "休"}],
    "2026-08-21": [{"person": "こうき", "summary": "出店"}, {"person": "ゆい", "summary": "休"}],
    "2026-08-22": [{"person": "こうき", "summary": "出店"}],
    "2026-08-23": [{"person": "こうき", "summary": "出店"}],
    "2026-08-25": [{"person": "ゆい", "summary": "休"}],
    "2026-08-26": [{"person": "こうき", "summary": "希望休"}, {"person": "ゆい", "summary": "出張"}],
    "2026-08-27": [{"person": "こうき", "summary": "希望休"}, {"person": "ゆい", "summary": "休"}],
    "2026-08-28": [{"person": "こうき", "summary": "休"}],
}
