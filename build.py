...
:root {
  ...
  /* こうき: meeting / outing で色違い */
  --kouki: #4f46e5;          /* 休系 メイン藍 */
  --kouki-meeting: #1e3a8a;  /* 会議: ネイビー(濃) */
  --kouki-outing: #38bdf8;   /* 出店: スカイブルー(明) */
  --kouki-soft: #eef2ff;
  --yui: #be185d;            /* 休系 ローズ */
  --yui-meeting: #9d174d;    /* 会議: 濃ローズ */
  --yui-outing: #f472b6;     /* 出店: ライトピンク */
  --yui-soft: #fce7f3;
  /* ケロッピーグリーン(給料日) */
  --pay: #65a30d;
  --pay-soft: #ecfccb;
  ...
  --sun: #dc2626;
  --sat: #2563eb;
  --shadow-sm: 0 1px 2px rgba(101, 163, 13, 0.06);
  --shadow-md: 0 4px 12px rgba(101, 163, 13, 0.1);
  --shadow-lg: 0 16px 48px rgba(101, 163, 13, 0.2);
...
.month-label .froggy {
  font-size: 20px;
  line-height: 1;
  filter: drop-shadow(0 1px 2px rgba(101, 163, 13, 0.3));
}
.icon-btn {
  width: 44px;
  height: 44px;
...
.today-btn {
  background: linear-gradient(135deg, var(--keroppi) 0%, var(--keroppi-light) 100%);
  color: white;
  font-weight: 800;
  padding: 8px 14px;
  border-radius: 999px;
...
/* 二人とも休みの日 */
.cell.both-off {
  background: linear-gradient(135deg, #fff1f9 0%, #fef3c7 50%, #ecfccb 100%);
...
.cell.both-off::after {
  ...
  right: 4px;
  font-size: 13px;
  filter: drop-shadow(0 1px 2px rgba(244, 114, 182, 0.4));
  pointer-events: none;
...
.event.kouki.t-meeting { background: var(--kouki-meeting); }
.event.kouki.t-outing { background: var(--kouki-outing); color: #0c4a6e; }
.event.kouki.t-outing .event-name { opacity: 0.7; }
.event.yui { background: var(--yui); }
.event.yui.t-meeting { background: var(--yui-meeting); }
.event.yui.t-outing { background: var(--yui-outing); color: #831843; }
.event.yui.t-outing .event-name { opacity: 0.7; }
.event.pay { background: linear-gradient(135deg, var(--pay) 0%, var(--keroppi-light) 100%); }
...
.fab {
  ...
  align-items: center;
  justify-content: center;
  box-shadow: 0 6px 20px rgba(101, 163, 13, 0.45), 0 2px 6px rgba(101, 163, 13, 0.3);
  z-index: 30;
...
<header>
  <div class="top-row">
    <div class="title-block">
      ...
      <div class="month-label"><span id="month-label"></span><span class="froggy">🐸</span></div>
    </div>
...
function render() {
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    if (isBothOff(builtinForDay)) cell.classList.add('both-off');
